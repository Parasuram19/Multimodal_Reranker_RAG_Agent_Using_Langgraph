...
# LangChain Splitters (Fixes the header chunking issue)
...
class FinancialIngestionPipeline:
    ...
    # ---- Parse PDF (FIXED: Markdown Header Splitter Pipeline) --------------
    def parse_document(self, file_path: str, base_meta: Dict[str, Any]) -> List[Dict[str, Any]]:
        """Parse PDF → Markdown → Header Split → Size Split → Metadata Enrichment."""
        result = self.converter.convert(file_path)
        doc = result.document
        base_meta["total_pages"] = len(doc.pages) if hasattr(doc, "pages") else 0
        
        # 1. Export to clean Markdown
        md_text = doc.export_to_markdown()
        if not md_text.strip():
            raise ValueError("Markdown export is empty. Check PDF quality or Docling config.")

        # 2. Split by document structure (H1, H2, H3)
        header_splitter = MarkdownHeaderTextSplitter(
            headers_to_split_on=[("#", "H1"), ("##", "H2"), ("###", "H3")],
            strip_headers=False  # Keep headers in content for better embedding context
        )
        header_chunks = header_splitter.split_text(md_text)

        # 3. Enforce CHUNK_SIZE/CHUNK_OVERLAP while preserving metadata
        ...
        final_docs = size_splitter.split_documents(header_chunks)

        # 4. Build pipeline-compatible chunks with enriched metadata
        chunks = []
        for idx, doc_obj in enumerate(final_docs):
            # Create a readable section path from header metadata
            section_parts = []
            if "H1" in doc_obj.metadata: section_parts.append(doc_obj.metadata["H1"])
            if "H2" in doc_obj.metadata: section_parts.append(doc_obj.metadata["H2"])
            if "H3" in doc_obj.metadata: section_parts.append(doc_obj.metadata["H3"])
            section_context = " > ".join(section_parts) if section_parts else "Unknown"
            ...
            meta = {
                **base_meta,
                ...
                "header_metadata": doc_obj.metadata,  # Full H1/H2/H3 dict for filtering
                "source_file": os.path.basename(file_path)
            }
            chunks.append({"content": doc_obj.page_content, "meta": meta})

        print(f"[parse] Generated {len(chunks)} chunks from {base_meta['total_pages']} pages")
        return chunks
    ...
    def process_and_store(self, file_path: str, base_meta: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 2. Embed
        texts = [c["content"] for c in chunks]
        embeddings = asyncio.run(self.embed_batch(texts))
        
        # 3. Store
        metas = [c["meta"] for c in chunks]
        ids = [f"{m['document_id']}_{m['chunk_index']}" for m in metas]
        self.vectorstore.add_embeddings(texts=texts, metadatas=metas, ids=ids, embeddings=embeddings)
        print(f"[store] Inserted {len(chunks)} chunks")

        # 4. Index
        self.create_index(lists=100)
        return {"status": "completed", "document_id": base_meta["document_id"], "indexed_chunks": len(chunks), "total_pages": base_meta["total_pages"]}
